...
export default function Message({ content, embeds, attachments }: Props) {
    ...
    return (
        <div class="flex flex-col pl-[40px] gap-2">
            <span class="text-[#A1A1AA] text-lg" dangerouslySetInnerHTML={{ __html: content.replaceAll("\n", "<br>") }}>
            </span>
            {embeds && embeds.map((embed) => {
                return (
                    <div class="flex flex-col gap-2 px-2 my-2 border-l-2 border-l-[#02F67C]">
                        ...
                        <iframe width="350" height="250" class="w-full h-[revert-layout]" src={embed.video.url}
            ...
            {attachments && attachments.map((img) => {
                return (
                    <Image width={350} height={250} class="w-full h-auto object-cover" src={img.url} alt={img.filename} />
                )
            })
